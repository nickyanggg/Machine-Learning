...
#     train_x, y = load_training_data(training_label)
#     train_x_no_label = load_training_data(training_unlabel)
...
def pre_testing(batch_size, test_loader, model, device, threshold):
    model.eval()
    ret_output = []
    with torch.no_grad():
...
# 處理好各個 data 的路徑
train_with_label = training_label
train_no_label = training_unlabel

w2v_path = './best_w2v_all.model' # 處理 word to vec model 的路徑
...
outputs = pre_testing(batch_size, test_loader, model, device, threshold)

remove_idx = []
for i in range(len(outputs)):
    if outputs[i] == 1 or outputs[i] == 0:
        remove_idx.append(1)
    else:
        remove_idx.append(0)

loc = torch.ByteTensor(remove_idx)
...
# 把 data 分為 training data 跟 validation data（將一部份 training data 拿去當作 validation data）
X_val, y_val = train_x[180000:], y[180000:]
...
all_x = torch.cat([X_train, test_x[loc]], dim=0)
all_y = torch.cat([y_train, (torch.LongTensor(outputs))[loc]], dim=0)
...
training(batch_size, epoch, lr, all_loader, val_loader, model, device)